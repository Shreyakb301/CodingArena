...
internal val advancedWorkoutSteps: List<WorkoutStep> = listOf(
    ...
    step(
        ...
        languageVariants = listOf(
            WorkoutCodeVariant(
                ...
                choices = listOf(
                    choice("if price - min_price > max_profit:\n    max_profit = price - min_price\n    buy_day = min_price_day\n    sell_day = day\nif price < min_price:\n    min_price = price\n    min_price_day = day", true, "Recording buy_day and sell_day only when a strictly better profit is found, using the minimum's own recorded day rather than the current day, correctly tracks which specific days produced the best profit in a single pass.", code = "if price - min_price > max_profit:\n    max_profit = price - min_price\n    buy_day = min_price_day\n    sell_day = day\nif price < min_price:\n    min_price = price\n    min_price_day = day"),
                    choice("if price < min_price:\n    min_price = price\n    min_price_day = day\nif price - min_price > max_profit:\n    max_profit = price - min_price\n    buy_day = min_price_day\n    sell_day = day", false, "Updating the minimum before checking profit means a new minimum day can be immediately compared against itself as both buy and sell day, producing a zero-profit 'best' pair on the very day the minimum updates.", code = "if price < min_price:\n    min_price = price\n    min_price_day = day\nif price - min_price > max_profit:\n    max_profit = price - min_price\n    buy_day = min_price_day\n    sell_day = day"),
                    choice("if price - min_price > max_profit:\n    max_profit = price - min_price\n    buy_day = day\n    sell_day = day\nif price < min_price:\n    min_price = price\n    min_price_day = day", false, "Setting buy_day to the current day instead of min_price_day records the wrong purchase day entirely - the buy day should be wherever the running minimum price actually occurred, not wherever the profit was just measured.", code = "if price - min_price > max_profit:\n    max_profit = price - min_price\n    buy_day = day\n    sell_day = day\nif price < min_price:\n    min_price = price\n    min_price_day = day"),
                ),
            ),
            WorkoutCodeVariant(
                JAVA,
                ...
                choices = listOf(
                    choice("if (price - minPrice > maxProfit) { maxProfit = price - minPrice; buyDay = minPriceDay; sellDay = day; }\nif (price < minPrice) { minPrice = price; minPriceDay = day; }", true, "Recording buyDay and sellDay only when a strictly better profit is found, using the minimum's own recorded day rather than the current day, correctly tracks which specific days produced the best profit in a single pass.", code = "if (price - minPrice > maxProfit) { maxProfit = price - minPrice; buyDay = minPriceDay; sellDay = day; }\nif (price < minPrice) { minPrice = price; minPriceDay = day; }"),
                    choice("if (price < minPrice) { minPrice = price; minPriceDay = day; }\nif (price - minPrice > maxProfit) { maxProfit = price - minPrice; buyDay = minPriceDay; sellDay = day; }", false, "Updating the minimum before checking profit means a new minimum day can be immediately compared against itself as both buy and sell day, producing a zero-profit 'best' pair on the very day the minimum updates.", code = "if (price < minPrice) { minPrice = price; minPriceDay = day; }\nif (price - minPrice > maxProfit) { maxProfit = price - minPrice; buyDay = minPriceDay; sellDay = day; }"),
                    choice("if (price - minPrice > maxProfit) { maxProfit = price - minPrice; buyDay = day; sellDay = day; }\nif (price < minPrice) { minPrice = price; minPriceDay = day; }", false, "Setting buyDay to the current day instead of minPriceDay records the wrong purchase day entirely - the buy day should be wherever the running minimum price actually occurred, not wherever the profit was just measured.", code = "if (price - minPrice > maxProfit) { maxProfit = price - minPrice; buyDay = day; sellDay = day; }\nif (price < minPrice) { minPrice = price; minPriceDay = day; }"),
                ),
            ),
            WorkoutCodeVariant(
                JAVASCRIPT,
            ...
            WorkoutCodeVariant(
                CPP,
                "int minPrice = INT_MAX;\nint minPriceDay = 0;\nint maxProfit = 0;\nint buyDay = 0;\nint sellDay = 0;\nfor (int day = 0; day < (int)prices.size(); day++) {\n    int price = prices[day];\n    // ???\n}",
                choices = listOf(
                    choice("if (price - minPrice > maxProfit) { maxProfit = price - minPrice; buyDay = minPriceDay; sellDay = day; }\nif (price < minPrice) { minPrice = price; minPriceDay = day; }", true, "Recording buyDay and sellDay only when a strictly better profit is found, using the minimum's own recorded day rather than the current day, correctly tracks which specific days produced the best profit in a single pass.", code = "if (price - minPrice > maxProfit) { maxProfit = price - minPrice; buyDay = minPriceDay; sellDay = day; }\nif (price < minPrice) { minPrice = price; minPriceDay = day; }"),
                    choice("if (price < minPrice) { minPrice = price; minPriceDay = day; }\nif (price - minPrice > maxProfit) { maxProfit = price - minPrice; buyDay = minPriceDay; sellDay = day; }", false, "Updating the minimum before checking profit means a new minimum day can be immediately compared against itself as both buy and sell day, producing a zero-profit 'best' pair on the very day the minimum updates.", code = "if (price < minPrice) { minPrice = price; minPriceDay = day; }\nif (price - minPrice > maxProfit) { maxProfit = price - minPrice; buyDay = minPriceDay; sellDay = day; }"),
                    choice("if (price - minPrice > maxProfit) { maxProfit = price - minPrice; buyDay = day; sellDay = day; }\nif (price < minPrice) { minPrice = price; minPriceDay = day; }", false, "Setting buyDay to the current day instead of minPriceDay records the wrong purchase day entirely - the buy day should be wherever the running minimum price actually occurred, not wherever the profit was just measured.", code = "if (price - minPrice > maxProfit) { maxProfit = price - minPrice; buyDay = day; sellDay = day; }\nif (price < minPrice) { minPrice = price; minPriceDay = day; }"),
            ...
            WorkoutCodeVariant(
                ...
                choices = listOf(
                    choice("if price-minPrice > maxProfit {\n    maxProfit = price - minPrice\n    buyDay = minPriceDay\n    sellDay = day\n}\nif price < minPrice {\n    minPrice = price\n    minPriceDay = day\n}", true, "Recording buyDay and sellDay only when a strictly better profit is found, using the minimum's own recorded day rather than the current day, correctly tracks which specific days produced the best profit in a single pass.", code = "if price-minPrice > maxProfit {\n    maxProfit = price - minPrice\n    buyDay = minPriceDay\n    sellDay = day\n}\nif price < minPrice {\n    minPrice = price\n    minPriceDay = day\n}"),
                    choice("if price < minPrice {\n    minPrice = price\n    minPriceDay = day\n}\nif price-minPrice > maxProfit {\n    maxProfit = price - minPrice\n    buyDay = minPriceDay\n    sellDay = day\n}", false, "Updating the minimum before checking profit means a new minimum day can be immediately compared against itself as both buy and sell day, producing a zero-profit 'best' pair on the very day the minimum updates.", code = "if price < minPrice {\n    minPrice = price\n    minPriceDay = day\n}\nif price-minPrice > maxProfit {\n    maxProfit = price - minPrice\n    buyDay = minPriceDay\n    sellDay = day\n}"),
                    choice("if price-minPrice > maxProfit {\n    maxProfit = price - minPrice\n    buyDay = day\n    sellDay = day\n}\nif price < minPrice {\n    minPrice = price\n    minPriceDay = day\n}", false, "Setting buyDay to the current day instead of minPriceDay records the wrong purchase day entirely - the buy day should be wherever the running minimum price actually occurred, not wherever the profit was just measured.", code = "if price-minPrice > maxProfit {\n    maxProfit = price - minPrice\n    buyDay = day\n    sellDay = day\n}\nif price < minPrice {\n    minPrice = price\n    minPriceDay = day\n}"),
                ),
            ),
            WorkoutCodeVariant(
                SWIFT,
    ...
    step(
        ...
        languageVariants = listOf(
            WorkoutCodeVariant(
                PYTHON,
                "# input arrives as a generator of characters, not a full string\ndef is_valid_streaming(char_source):\n    # ???",
                choices = listOf(
                    choice("stack = []\npairs = {\")\": \"(\", \"]\": \"[\", \"}\": \"{\"}\nfor c in char_source:\n    if c in \"([{\":\n        stack.append(c)\n    else:\n        if not stack or stack.pop() != pairs[c]:\n            return False\nreturn len(stack) == 0", true, "The stack-based algorithm never actually needed random access to the whole string at once - it only ever looks at one character at a time and the current stack top, which makes it naturally streaming-compatible with no restructuring beyond swapping the iteration source.", code = "stack = []\npairs = {\")\": \"(\", \"]\": \"[\", \"}\": \"{\"}\nfor c in char_source:\n    if c in \"([{\":\n        stack.append(c)\n    else:\n        if not stack or stack.pop() != pairs[c]:\n            return False\nreturn len(stack) == 0"),
                    choice("all_chars = list(char_source)\nreturn is_valid(\"\".join(all_chars))", false, "Collecting the entire stream into a list before processing defeats the purpose of streaming entirely, since it still requires holding the whole input in memory at once, exactly what needed to be avoided.", code = "all_chars = list(char_source)\nreturn is_valid(\"\".join(all_chars))"),
                    choice("stack = []\nfor c in char_source:\n    stack.append(c)\nreturn len(stack) % 2 == 0", false, "Just checking whether the total character count is even doesn't verify anything about matching types or correct nesting order - it would accept clearly invalid strings like \"))((\" as long as the length happens to be even.", code = "stack = []\nfor c in char_source:\n    stack.append(c)\nreturn len(stack) % 2 == 0"),
                ),
            ),
            WorkoutCodeVariant(
                JAVA,
                "// input arrives as an Iterator<Character>, not a full String\nboolean isValidStreaming(Iterator<Character> charSource) {\n    // ???\n}",
                choices = listOf(
                    choice("Deque<Character> stack = new ArrayDeque<>();\nMap<Character, Character> pairs = Map.of(')', '(', ']', '[', '}', '{');\nwhile (charSource.hasNext()) {\n    char c = charSource.next();\n    if (\"([{\".indexOf(c) >= 0) stack.push(c); else { if (stack.isEmpty() || stack.pop() != pairs.get(c)) return false; }\n}\nreturn stack.isEmpty();", true, "The stack-based algorithm never actually needed random access to the whole string at once - it only ever looks at one character at a time and the current stack top, which makes it naturally streaming-compatible with no restructuring beyond swapping the iteration source.", code = "Deque<Character> stack = new ArrayDeque<>();\nMap<Character, Character> pairs = Map.of(')', '(', ']', '[', '}', '{');\nwhile (charSource.hasNext()) {\n    char c = charSource.next();\n    if (\"([{\".indexOf(c) >= 0) stack.push(c); else { if (stack.isEmpty() || stack.pop() != pairs.get(c)) return false; }\n}\nreturn stack.isEmpty();"),
                    choice("List<Character> allChars = new ArrayList<>();\ncharSource.forEachRemaining(allChars::add);\nreturn isValid(allChars);", false, "Collecting the entire stream into a list before processing defeats the purpose of streaming entirely, since it still requires holding the whole input in memory at once, exactly what needed to be avoided.", code = "List<Character> allChars = new ArrayList<>();\ncharSource.forEachRemaining(allChars::add);\nreturn isValid(allChars);"),
                    choice("Deque<Character> stack = new ArrayDeque<>();\nwhile (charSource.hasNext()) stack.push(charSource.next());\nreturn stack.size() % 2 == 0;", false, "Just checking whether the total character count is even doesn't verify anything about matching types or correct nesting order - it would accept clearly invalid strings like \"))((\" as long as the length happens to be even.", code = "Deque<Character> stack = new ArrayDeque<>();\nwhile (charSource.hasNext()) stack.push(charSource.next());\nreturn stack.size() % 2 == 0;"),
                ),
            ),
            WorkoutCodeVariant(
                JAVASCRIPT,
                "// input arrives as an iterator of characters, not a full string\nfunction isValidStreaming(charSource) {\n    // ???\n}",
                choices = listOf(
                    choice("const stack = [];\nconst pairs = { ')': '(', ']': '[', '}': '{' };\nfor (const c of charSource) {\n    if (\"([{\".includes(c)) stack.push(c); else { if (stack.length === 0 || stack.pop() !== pairs[c]) return false; }\n}\nreturn stack.length === 0;", true, "The stack-based algorithm never actually needed random access to the whole string at once - it only ever looks at one character at a time and the current stack top, which makes it naturally streaming-compatible with no restructuring beyond swapping the iteration source.", code = "const stack = [];\nconst pairs = { ')': '(', ']': '[', '}': '{' };\nfor (const c of charSource) {\n    if (\"([{\".includes(c)) stack.push(c); else { if (stack.length === 0 || stack.pop() !== pairs[c]) return false; }\n}\nreturn stack.length === 0;"),
                    choice("const allChars = [...charSource];\nreturn isValid(allChars.join(\"\"));", false, "Collecting the entire stream into an array before processing defeats the purpose of streaming entirely, since it still requires holding the whole input in memory at once, exactly what needed to be avoided.", code = "const allChars = [...charSource];\nreturn isValid(allChars.join(\"\"));"),
                    choice("const stack = [];\nfor (const c of charSource) stack.push(c);\nreturn stack.length % 2 === 0;", false, "Just checking whether the total character count is even doesn't verify anything about matching types or correct nesting order - it would accept clearly invalid strings like \"))((\" as long as the length happens to be even.", code = "const stack = [];\nfor (const c of charSource) stack.push(c);\nreturn stack.length % 2 === 0;"),
                ),
            ),
            WorkoutCodeVariant(
                CPP,
                "// input arrives via an input stream, not a full string\nbool isValidStreaming(istream& charSource) {\n    // ???\n}",
                choices = listOf(
                    choice("vector<char> stack;\nunordered_map<char, char> pairs = {{')', '('}, {']', '['}, {'}', '{'}};\nchar c;\nwhile (charSource.get(c)) {\n    if (string(\"([{\").find(c) != string::npos) {\n        stack.push_back(c);\n    } else {\n        if (stack.empty()) return false;\n        char top = stack.back(); stack.pop_back();\n        if (top != pairs[c]) return false;\n    }\n}\nreturn stack.empty();", true, "The stack-based algorithm never actually needed random access to the whole string at once - it only ever looks at one character at a time and the current stack top, which makes it naturally streaming-compatible with no restructuring beyond swapping the iteration source.", code = "vector<char> stack;\nunordered_map<char, char> pairs = {{')', '('}, {']', '['}, {'}', '{'}};\nchar c;\nwhile (charSource.get(c)) {\n    if (string(\"([{\").find(c) != string::npos) {\n        stack.push_back(c);\n    } else {\n        if (stack.empty()) return false;\n        char top = stack.back(); stack.pop_back();\n        if (top != pairs[c]) return false;\n    }\n}\nreturn stack.empty();"),
                    choice("string all((istreambuf_iterator<char>(charSource)), istreambuf_iterator<char>());\nreturn isValid(all);", false, "Collecting the entire stream into a string before processing defeats the purpose of streaming entirely, since it still requires holding the whole input in memory at once, exactly what needed to be avoided.", code = "string all((istreambuf_iterator<char>(charSource)), istreambuf_iterator<char>());\nreturn isValid(all);"),
                    choice("vector<char> stack;\nchar c;\nwhile (charSource.get(c)) stack.push_back(c);\nreturn stack.size() % 2 == 0;", false, "Just checking whether the total character count is even doesn't verify anything about matching types or correct nesting order - it would accept clearly invalid strings like \"))((\" as long as the length happens to be even.", code = "vector<char> stack;\nchar c;\nwhile (charSource.get(c)) stack.push_back(c);\nreturn stack.size() % 2 == 0;"),
                ),
            ),
            WorkoutCodeVariant(
                GO,
                "// input arrives on a channel of runes, not a full string\nfunc isValidStreaming(charSource <-chan rune) bool {\n    // ???\n}",
                choices = listOf(
                    choice("stack := []rune{}\npairs := map[rune]rune{')': '(', ']': '[', '}': '{'}\nfor c := range charSource {\n    if strings.ContainsRune(\"([{\", c) {\n        stack = append(stack, c)\n    } else {\n        if len(stack) == 0 || stack[len(stack)-1] != pairs[c] {\n            return false\n        }\n        stack = stack[:len(stack)-1]\n    }\n}\nreturn len(stack) == 0", true, "The stack-based algorithm never actually needed random access to the whole string at once - it only ever looks at one character at a time and the current stack top, which makes it naturally streaming-compatible with no restructuring beyond swapping the iteration source.", code = "stack := []rune{}\npairs := map[rune]rune{')': '(', ']': '[', '}': '{'}\nfor c := range charSource {\n    if strings.ContainsRune(\"([{\", c) {\n        stack = append(stack, c)\n    } else {\n        if len(stack) == 0 || stack[len(stack)-1] != pairs[c] {\n            return false\n        }\n        stack = stack[:len(stack)-1]\n    }\n}\nreturn len(stack) == 0"),
                    choice("var all []rune\nfor c := range charSource {\n    all = append(all, c)\n}\nreturn isValid(string(all))", false, "Collecting the entire stream into a slice before processing defeats the purpose of streaming entirely, since it still requires holding the whole input in memory at once, exactly what needed to be avoided.", code = "var all []rune\nfor c := range charSource {\n    all = append(all, c)\n}\nreturn isValid(string(all))"),
                    choice("stack := []rune{}\nfor c := range charSource {\n    stack = append(stack, c)\n}\nreturn len(stack)%2 == 0", false, "Just checking whether the total character count is even doesn't verify anything about matching types or correct nesting order - it would accept clearly invalid strings like \"))((\" as long as the length happens to be even.", code = "stack := []rune{}\nfor c := range charSource {\n    stack = append(stack, c)\n}\nreturn len(stack)%2 == 0"),
                ),
            ),
            WorkoutCodeVariant(
                SWIFT,
                "// input arrives as an iterator of characters, not a full string\nfunc isValidStreaming(_ charSource: AnyIterator<Character>) -> Bool {\n    // ???\n}",
                choices = listOf(
                    choice("var stack: [Character] = []\nlet pairs: [Character: Character] = [\")\": \"(\", \"]\": \"[\", \"}\": \"{\"]\nwhile let c = charSource.next() {\n    if \"([{\".contains(c) {\n        stack.append(c)\n    } else {\n        if stack.isEmpty || stack.removeLast() != pairs[c] { return false }\n    }\n}\nreturn stack.isEmpty", true, "The stack-based algorithm never actually needed random access to the whole string at once - it only ever looks at one character at a time and the current stack top, which makes it naturally streaming-compatible with no restructuring beyond swapping the iteration source.", code = "var stack: [Character] = []\nlet pairs: [Character: Character] = [\")\": \"(\", \"]\": \"[\", \"}\": \"{\"]\nwhile let c = charSource.next() {\n    if \"([{\".contains(c) {\n        stack.append(c)\n    } else {\n        if stack.isEmpty || stack.removeLast() != pairs[c] { return false }\n    }\n}\nreturn stack.isEmpty"),
                    choice("var allChars: [Character] = []\nwhile let c = charSource.next() { allChars.append(c) }\nreturn isValid(String(allChars))", false, "Collecting the entire stream into an array before processing defeats the purpose of streaming entirely, since it still requires holding the whole input in memory at once, exactly what needed to be avoided.", code = "var allChars: [Character] = []\nwhile let c = charSource.next() { allChars.append(c) }\nreturn isValid(String(allChars))"),
                    choice("var stack: [Character] = []\nwhile let c = charSource.next() { stack.append(c) }\nreturn stack.count % 2 == 0", false, "Just checking whether the total character count is even doesn't verify anything about matching types or correct nesting order - it would accept clearly invalid strings like \"))((\" as long as the length happens to be even.", code = "var stack: [Character] = []\nwhile let c = charSource.next() { stack.append(c) }\nreturn stack.count % 2 == 0"),
                ),
            ),
        ),
    ),
